...
6. Personal profile — `{RESUME_ROOT}/profile.yml` (optional; see **Personal profile**)
7. Profile template — `{RESUME_ROOT}/profile.example.yml`
...
{AGENT_SKILLS_ROOT}/resume/
  profile.yml                           # personal skills inventory (optional, gitignored)
...
### 3b. Load personal profile (optional)

1. If `{RESUME_ROOT}/profile.yml` exists → parse it. Else skip (current behavior).
2. If missing and user may benefit → mention they can `cp profile.example.yml profile.yml`.
3. Record from profile when present:
   - `identity.default_role`, `identity.location`
   - `skills` categories, `export_rules`, `senior_framing`
4. Do **not** read profile during evidence gathering — repo PRs/commits remain the only source for features and ownership.
...
4. Fill every section of the template structure you can support with evidence:
   ...
   - **Skills enrichment (when profile loaded):** see **Personal profile** below.
...
- Whether `profile.yml` was loaded and skills enriched

## Personal profile (`resume/profile.yml`)

Optional gitignored file. Copy from `profile.example.yml`. Used to enrich exports — **evidence first**.

### Enrichment algorithm (Skills line + Short CV stack line)

1. **Start** with skills inferred from repo only (Tech used, package.json, PR paths).
2. **Scan** repo doc text + repo stack for keywords matching `export_rules.category_triggers` (case-insensitive).
3. For each matched trigger → append skills from that profile category **not** in `export_rules.export_exclude`.
4. **Always include** `export_rules.core_always_include` when repo doc mentions TypeScript/React or equivalent stack.
5. **Dedupe** and **cap** at `export_rules.max_skills_ats` for ATS; use same list for LinkedIn Skills (cap `max_skills_linkedin` if set).
6. **Never add** a skill from profile unless (a) repo evidence mentions the trigger keyword, or (b) it is in `core_always_include` and repo is clearly TypeScript/React.

### Role and bullet tone

- **Role / context:** use repo-stated role when present; else `identity.default_role`.
- **Senior framing:** when `senior_framing.enabled`, prefer verbs from `senior_framing.verbs` on bullets where PR evidence shows ownership, cross-cutting work, or architectural scope — do not upgrade chore-only commits.
...
- Per-repo output is a **source doc** for one experience entry — not a full multi-page CV. Export blocks are for manual merge into a consolidated resume later, or use `/merge-resume-experience` to merge multiple repos for the same employer.
